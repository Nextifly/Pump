...
import passport_pump, {
	IPassportPump,
	IPassportPRCData1,
	IPassport1,
} from '../(pages)/pump/[id]/passport/passport'
...
interface PassportStore {
  ...
  deletePRC: (pumpId: number, recordId: number) => void
}
...
export const usePassportStore = create<PassportStore>()(
	persist(
		set => ({
			...
			deletePRC: (pumpId: number, recordId: number) =>
				set(state => ({
					...state,
					passports: {
						...state.passports,
						passport_prc: state.passports.passport_prc.map(p =>
							p.id === pumpId
								? {
										...p,
										data: p.data.filter(item => item.id !== recordId),
									}
								: p,
						),
					},
