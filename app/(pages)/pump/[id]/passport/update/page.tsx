...
export default function EditPage() {
	...
	const updatePassport = usePassportStore(state => state.updatePump) // Убедитесь, что в сторе метод называется update

	const originalData = passports.passport_pump.find(p => p.id === id)
	...
	const handleChange = (
		section: 'specification' | 'property',
		key: string,
		value: string,
	) => {
		setFormData(prev => ({
								...
								<input
									...
									value={
										formData.specification[
											el.key as keyof typeof formData.specification
										]
									}
									onChange={e =>
										handleChange('specification', el.key, e.target.value)
									}
								/>
							</div>
						))}
						...
						{productData.map(el => (
							...
							>
								...
								<input
									...
									className='bg-slate-900/50 border border-slate-700 p-2 rounded outline-none text-white'
									value={
										formData.property[el.key as keyof typeof formData.property]
									}
									onChange={e =>
										handleChange('property', el.key, e.target.value)
									}
								/>
							</div>
						))}
